...
public class App {
	...
	private static void printclimate() {
		
		System.out.println("SPANISH CLIMATE DATA FROM 31/10/2017");
		System.out.println("**************************************************\n\n");
		maximaYMinimaTotal();
		maximaPorProvincias();
		minimaPorProvincias();
	...
	private static void datosPorProvincia(String string) {
		...
		data.stream()
			.sorted(Comparator.comparing(DataReg::getStation))
			.forEach((r) -> System.out.printf(
						"Meteo station: %-35s | Max: % 6.2f (%s) | Min: % 6.2f (%s) | Rainfall: %.2f%n",
						r.getStation(), r.getMax().getTemp() , r.getMax().getTime(),
						r.getMin().getTemp(), r.getMin().getTime(), r.getRain()));
